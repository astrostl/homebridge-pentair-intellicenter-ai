...
export class ConfigValidator {
  ...
  private static validateOptionalFields(config: PlatformConfig, warnings: string[], sanitizedConfig: PentairConfig) {
    ...
    // Heat mode override for multi-mode heaters (0 = auto-detect, 7-10 = specific modes)
    if (config.heatModeOverride !== undefined && config.heatModeOverride !== null && config.heatModeOverride !== '') {
      ...
      if (!isNaN(modeValue) && modeValue >= 1 && modeValue <= 15) {
        ...
        warnings.push(`heatModeOverride must be 0 (auto) or a number between 1 and 15, ignoring value: ${config.heatModeOverride}`);
      }
    }
